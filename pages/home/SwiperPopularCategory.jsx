...
export default function SwiperPopularCategory({ shop_tabs }) {
    ...
    return (
        ...
        <div ref={shop_tabs} id='x' className="relative  overflow-x-scroll flex justify-start items-center space-x-2  md:space-x-5 mt-3 md:mt-0 px-8">
            {home_data?.category?.map((item, index) => (
                <div className='w-[550px]' key={index}>
                    <Card02
                        ...
                        // stars={item?.rate}
                        // text={item?.details ?? ''}
                        total_cards={item?.card_count ?? 0}
                        onClick={() => router.push(`/post-cards?category_index=${index}`)}
                    />
                </div>
            ))}
        </div>
